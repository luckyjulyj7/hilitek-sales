...
export default function PolicyPage({ pageKey }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 font-sans">
      ...
      {page.intro && <p className="mt-3 text-ink/75 text-[15px] leading-relaxed">{page.intro}</p>}
        ...
        {(page.sections || []).map((s, i) => (
          <section key={i} className="border-l-[3px] border-yellow pl-4">
            ...
            {Array.isArray(s.body)
              ? s.body.map((b, j) => (
                  <p key={j} className="mt-2 text-[15px] text-ink/80 leading-relaxed">{b}</p>
                ))
              : s.body && <p className="mt-2 text-[15px] text-ink/80 leading-relaxed">{s.body}</p>}
            {s.bullets?.length > 0 && (
              <ul className="mt-2 list-disc pl-5 space-y-1.5 text-[15px] text-ink/80 leading-relaxed">
      ...
      <div className="mt-10 text-[14px] text-mute">
...
function BankCard({ bank }) {
  return (
    <div className="mt-6 border border-line rounded-lg bg-white p-5">
      ...
      <dl className="mt-3 grid grid-cols-[110px_1fr] gap-y-2 text-[15px]">
      ...
      {bank.note && (
        <p className="mt-3 pt-3 border-t border-line text-[14px] text-ink/70 leading-relaxed">
