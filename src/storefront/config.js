...
/**
 * Cây danh mục cho mega-menu (bố cục kiểu SUNTECH).
 * `slug` của nhóm và `cat` (tên danh mục con) phải khớp field `group` / `category`
 * trong dữ liệu sản phẩm (mockCatalog.js hoặc API sau này).
 * `icon` = tên icon lucide-react (danh sách gợi ý trong components/groupIcons.js).
 */
export const MENU = [
  {
    group: "Linh kiện PC",
    ...
    columns: [
      ...
      { heading: "Đồ hoạ & nguồn", items: ["Card màn hình", "Nguồn máy tính", "Tản nhiệt"] },
    ],
  },
  {
    group: "Gaming Gear",
    ...
    icon: "Gamepad2",
    columns: [
      { heading: "Nhập liệu", items: ["Bàn phím", "Chuột", "Lót chuột"] },
      { heading: "Âm thanh", items: ["Tai nghe"] },
    ],
  },
  {
    ...
    icon: "HardDrive",
    columns: [
      { heading: "Trong máy", items: ["Ổ cứng SSD", "Ổ cứng HDD"] },
      { heading: "Di động", items: ["SSD di động", "USB & Thẻ nhớ"] },
    ],
  },
  {
    group: "Màn hình",
    slug: "man-hinh",
    icon: "Monitor",
    columns: [{ heading: "Theo nhu cầu", items: ["Màn hình gaming", "Màn hình văn phòng"] }],
  },
  {
    group: "Phần mềm & Gia dụng",
    slug: "phan-mem-gia-dung",
    icon: "AppWindow",
    columns: [{ heading: "Khác", items: ["Phần mềm bản quyền", "Gia dụng"] }],
  },
];

/** Tra nhanh: tên danh mục -> nhóm cha. */
export const CATEGORY_TO_GROUP = MENU.reduce((acc, g) => {
  g.columns.forEach((col) => col.items.forEach((c) => (acc[c] = g.group)));
  return acc;
}, {});
